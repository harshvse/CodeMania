        ...
        <>
            <tbody className='text-white'>
                {problems.map((doc, index) => {
                    ...
                    return (
                        <tr className={`${index % 2 == 1 ? 'bg-dark-layer-1' : ''}`} key={doc.id}>
                            ...
                            <td className={`px-6 py-4`}>
                                {doc.videoId ? (<AiFillYoutube onClick={() => { setYoutubePlayer({ isOpen: true, videoId: doc.videoId as string }); console.log(doc.videoId) }} fontSize="28" className="cursor-pointer hover:text-red-600" />) : (<p className='text-gray-400'>Coming Soon...</p>)}
                            </td>
